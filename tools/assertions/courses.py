...
def assert_create_course_response(
    ...
    """
    Проверяет на соответствие запроса на создание курса с запросом
    :param request: Сериализованный запрос на создание курса
    :param response: Сериализованный ответ от API
    :raises AssertionError: В случае, если хотя бы одно поле не совпадает
    """
    assert_equal(response.course.title, request.title, "title")
    assert_equal(response.course.max_score, request.max_score, "max_score")
    assert_equal(response.course.min_score, request.min_score, "min_score")
